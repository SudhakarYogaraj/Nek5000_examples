...
!     namelists
      namelist /STATS/ stat_comp,stat_outp,stat_ifstpts
...
!     Seems weird to have ATIME,TSTART etc initialized in a different time step.
!     Initializing at ISTEP.eq.0 has issues when doing restart.
!     If checkpoint is called after time initialization then there is a discrepency
!     between STAT_TSTART and simulation TIME.
!     When done at ISTEP.eq.1 then it is not dependent on the order of calls for
!     checkpoint and STAT_AVG_ALL
...
!     update timing and counters
      ...
      STAT_EVN = STAT_EVN + 1
      STAT_EVRN =  STAT_EVRN + 1
      STAT_TACC = STAT_TACC + DT
